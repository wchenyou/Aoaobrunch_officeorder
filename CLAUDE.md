...
**送單後才能標記「已完成」，主揪跟店家是各自獨立的兩個旗標，2026-09-11 拆開的。** `sessions.status` 只剩「收單中」「已送單」兩種值，「已完成」不再是共用的 status。主揪在 `admin.html` 標記的是 `organizer_closed`（代表他自己結案了，例如收完錢），店家在 `vendor.html` 標記的是 `vendor_done`（代表出餐/準備進度）——兩邊互不影響，誰標記都不會動到另一邊的畫面。主揪標記完成後這團才會從首頁「你開過的團」消失；店家標記完成後這團才會從「待處理訂單」消失。改這塊邏輯時不要又把兩個欄位合併回同一個 status，這正是拆開前的問題。
...
- 管理頁與店家後台的訂單列表是輪詢／手動重新整理，不是推播；但店家後台有另外做「新訂單瀏覽器推播」（Web Push，見下面），送單那一刻會主動通知，不用等下次輪詢或手動整理。
- localStorage 只在單一裝置有效，換裝置靠信件。
- 寄信用的 Gmail 應用程式密碼 2026-09-14 已經設定好（寄件人 sunny30248@gmail.com），真的測試過會收到信。
- **店家後台新訂單瀏覽器推播（Web Push）**：送單那一刻（`finalizeSession`）會嘗試推播給所有訂閱過的裝置。VAPID 金鑰放在 `settings` 表（`網頁推播VAPID公鑰`／`網頁推播VAPID私鑰`），訂閱資訊存在 `push_subscriptions` 表。前端是 `docs/sw.js`（service worker）+ `vendor.html` 標頭的「開啟通知」按鈕，要店家自己在要用的裝置上按過一次才會生效——這一步沒辦法幫使用者代勞。
